...
Once the training job successfully completed, we can take a look at the model performance metrics. Amazon Rekognition Custom Labels provides various model performance metrics for (1) the entire test set and (2) for each image in your test set. 
...
eval_results
```
The F1 score is 0.646 for our Swoosh detector model. All other model performance metrics are only accessible via the Console or via downloading the evaluation results summary file which we will do below: 
...
```
We see that 56 images went into the training set and 14 images into the test set. This matches the 80/20 split of our data set of 70 images when we set `AutoCreate = TRUE` during the training job specification above. 

The parsed evaluation summary results also include various other model performance metrics besides just the F1 score:
...
According to the [official documentation](https://docs.aws.amazon.com/rekognition/latest/customlabels-dg/tr-metrics-use.html) all model performance metrics are calculated in the usual manner. The `Threshold` above is the value with which a prediction is counted as a true or false positive and is set based on the test set. 

Each image of the test set had one or more ground truth bounding boxes. The Swoosh object detector as an object detection model predicts bounding boxes in an image. Each predicted bounding box is associated with a confidence score. There are three types of bounding box predictions:
...
We will have a look at the individual bounding box predictions in each image of the test set. For this, we will download and parse another JSON file that includes the details for each predicted bounding box:
...
test_set_results <- s3$get_object(Bucket = custom_labels_bucket,
  ...
  .$Body %>% 
  rawToChar() %>%
  # The next three lines transform ndjson formatted response into json
...
```{r}
object_IDs <- test_set_results$`rekognition-custom-labels-object-id`
...
l <- list(.x = object_IDs, .y = object_list, .z = file_names)
...
eval_test_set_df <- pmap_dfr(l, function(.x, .y, .z) {
       df <- .y$`rekognition-custom-labels-evaluation-details` %>% 
         ...
         relocate(file_name, object_id, box_id)
       df
     })
...
```{r echo=FALSE, out.width = "85%"}
knitr::include_graphics("./images/console/10_evaluate_test_set_image.png")
```
